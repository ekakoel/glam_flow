<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-stone-800 leading-tight">Billing</h2>
    </x-slot>
    ...
    <div class="py-8 bg-gradient-to-b from-rose-50 via-amber-50 to-white min-h-screen">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-rose-100 shadow p-6">
                <h3 class="text-lg font-semibold text-stone-900">Current Subscription</h3>
                <div class="mt-4 grid md:grid-cols-2 gap-4">
                    <div class="p-4 rounded-xl bg-rose-50/40 border border-rose-100">
                        <p class="text-sm text-stone-500">Plan</p>
                        <p class="text-xl font-bold text-stone-900 uppercase">{{ $subscription?->plan ?? 'free' }}</p>
                    </div>
                    <div class="p-4 rounded-xl bg-rose-50/40 border border-rose-100">
                        <p class="text-sm text-stone-500">Trial / Expiry</p>
                        <p class="text-xl font-bold text-stone-900">
                            {{ $subscription?->expired_at?->format('d M Y') ?? 'No expiry' }}
                        </p>
                    </div>
                </div>

                <div class="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                    Plan upgrade akan membantu membuka limit booking dan fitur premium untuk operasional harian.
                </div>
