<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Buat Layanan</h2>
    </x-slot>
                ...
                <form method="POST" action="{{ route('admin.services.store') }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="mt-1 w-full rounded-md border-gray-300">
                            @error('price') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Durasi (menit)</label>
                            <input type="number" name="duration" value="{{ old('duration') }}" class="mt-1 w-full rounded-md border-gray-300">
                    ...
                    <div class="flex justify-end gap-3">
                        <a href="{{ route('admin.services.index') }}" class="px-4 py-2 border border-gray-300 rounded-md text-gray-700">Batal</a>
